    ...
    <div class="col-md-9">
      <div class="card">
        <div class="card-body">
          ...
          <form method="POST" action="{{ route('teacher.dates' , $teacher->id) }}" accept-charset="UTF-8" class="form-horizontal" enctype="multipart/form-data">
            {{ csrf_field() }}
            ...
            <div class="form-group {{ $errors->has('days') || $errors->has('time') ? 'has-error' : ''}}">
              ...
              <select name="days" class="form-control" id="days" >
                ...
                @for ($i = 1 ; $i <= 7 ; $i++) <option value="{{$i}}" {{$i == old('days') ? 'selected': ''}}>{{$teacher->getDaysAttribute($i)}}</option>
                  @endfor
              </select>
