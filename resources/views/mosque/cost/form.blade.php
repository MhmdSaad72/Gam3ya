<div class="form-group {{ $errors->has('price') ? 'has-error' : ''}}">
    ...
    <input class="form-control" name="price" type="text" id="price" value="{{ isset($cost->price) ? $cost->price : old('price')}}" >
...
<div class="form-group {{ $errors->has('mosque_id') ? 'has-error' : ''}}">
    ...
    <select name="mosque_id" class="form-control" id="mosque_id" >
      ...
      @foreach($mosques as $mosque)
        @if (isset($cost->mosque_id))
          <option value="{{$mosque->id}}"{{$mosque->id == $cost->mosque_id ? 'selected': ''}}>{{$mosque->name}}</option>
        @else
