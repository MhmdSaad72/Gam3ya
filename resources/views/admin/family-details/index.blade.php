                            ...
                            <table class="table">
                                ...
                                @foreach($familydetails as $item)
                                    <tr>
                                        ...
                                        <td>
                                            <a href="{{ url('/admin/family-details/' . $item->id) }}" title="View FamilyDetail"><button class="btn btn-info btn-sm"><i class="fa fa-eye" aria-hidden="true"></i> عرض</button></a>
                                            <a href="{{ url('/admin/family-details/' . $item->id . '/edit') }}" title="Edit FamilyDetail"><button class="btn btn-primary btn-sm"><i class="fa fa-pencil-square-o" aria-hidden="true"></i> تعديل</button></a>
                                            @if ($item->children->count() == 0)
                                              <form method="POST" action="{{ url('/admin/family-details' . '/' . $item->id) }}" accept-charset="UTF-8" style="display:inline">
                                                {{ method_field('DELETE') }}
                                                {{ csrf_field() }}
