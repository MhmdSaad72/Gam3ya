            ...
            <div class="col-md-9">
                <div class="card">
                    <div class="card-body">
                        ...
                        <form method="POST" action="{{ url('/admin/children') }}" accept-charset="UTF-8" class="form-horizontal" enctype="multipart/form-data">
                            {{ csrf_field() }}
                            @include ('admin.children.form', ['formMode' => 'create'])
                            ...
                            <div class="form-group {{ $errors->has('orphan_sponser_id') ? 'has-error' : ''}}">
                                <label for="orphan_sponser_id" class="control-label">*كافل اليتيم</label>
                                <select name="orphan_sponser_id" class="form-control" id="orphan_sponser_id" >
                                  <option value="" selected disabled>اختر كافل اليتيم</option>
                                  @foreach($orphanSponsers as $orphanSponser)
                                    <option value="{{$orphanSponser->id}}"{{$orphanSponser->id == old('orphan_sponser_id') ? 'selected': ''}}>{{$orphanSponser->name}}</option>
                                  @endforeach
